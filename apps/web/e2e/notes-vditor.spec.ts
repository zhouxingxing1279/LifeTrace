...
async function installMocks(page: Page, pushes: PushBody[], cloudMarkdown = "# Cloud note\n\nCloud body") {
  await page.route("**/api/v1/**", async (route) => {
    ...
    if (path === "/api/v1/sync/snapshot") return json(route, { snapshotId: "snapshot-1", snapshotCursor: "cursor-1", items: [{ entityType: "note.note", entityId: "note-1", serverVersion: "1", payload: { meta: meta("note-1"), noteType: "quick", title: "Vditor note", contentMarkdown: cloudMarkdown, contentText: "Cloud note Cloud body", contentHtml: "", contentJson: { type: "markdown", source: cloudMarkdown }, summary: "Cloud note Cloud body", isPinned: false, isFavorite: false, isArchived: false, folderId: null } }], nextPageToken: null, completed: true });
...
function noteMarkdownPush(pushes: PushBody[]) {
  for (const body of pushes) for (const change of body.changes ?? []) if (change.entityType === "note.note" && typeof change.payload?.contentMarkdown === "string") return change.payload.contentMarkdown;
  return null;
}
...
test("Vditor edits Markdown and autosaves the note to LifeTrace Cloud", async ({ page }) => {
  ...
  await editor.fill("# Local Vditor edit\n\n- [x] cloud autosave");
