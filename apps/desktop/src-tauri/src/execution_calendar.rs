...
use crate::{
    database::{
        ...
        repositories::{
            ...
            execution_structure::{
                self as recurrence_repository, RecurrenceRuleRecord, RecurrenceRuleWrite,
            },
        },
    },
...
fn matches_query(event: &CalendarEventRecord, query: &CalendarQuery) -> ExecutionResult<bool> {
    if event.is_all_day {
        ...
        let event_start = event
            .start_local_date
            .as_deref()
            .ok_or_else(|| storage("全天事件缺少 start_local_date"))?;
        let event_end = event
            .end_local_date
            .as_deref()
            .ok_or_else(|| storage("全天事件缺少 end_local_date"))?;
        ...
        let event_start = event
            .start_at
            .as_deref()
            .ok_or_else(|| storage("定时事件缺少 start_at"))?;
        let event_end = event
            .end_at
            .as_deref()
            .ok_or_else(|| storage("定时事件缺少 end_at"))?;
...
pub fn list_events(
    connection: &Connection,
    query: CalendarQuery,
) -> ExecutionResult<Vec<CalendarEventRecord>> {
...
pub fn find_conflicts(
    ...
    for event in events {
        ...
        for occurrence in
            repository::list_occurrences(connection, &user_id, &event.id).map_err(storage)?
        {
    ...
    match input.frequency.as_str() {
        ...
        "weekly" if input.weekdays.is_empty() || input.month_day.is_some() => {
            return Err(validation(
                "weekly 规则必须设置 weekdays，且不能设置 monthDay",
            ));
        }
        "monthly" if !input.weekdays.is_empty() || !matches!(input.month_day, Some(1..=31)) => {
            return Err(validation(
                "monthly 规则必须设置 1..=31 的 monthDay，且不能设置 weekdays",
            ));
        }
        _ => {}
    }
...
pub fn clear_event_recurrence(connection: &Connection, event_id: &str) -> ExecutionResult<()> {
    ...
    let transaction = connection
        .unchecked_transaction()
        .map_err(|error| storage(error.to_string()))?;
    repository::set_event_recurrence_rule(&transaction, &user_id, event_id, None)
        .map_err(storage)?;
...
pub fn change_occurrence_status(
    ...
    if !matches!(input.status.as_str(), "scheduled" | "cancelled" | "skipped") {
        return Err(validation(
            "occurrence 状态必须是 scheduled/cancelled/skipped",
        ));
    }
    let user_id = active_user(connection)?;
    get_event(connection, event_id)?;
...
mod tests {
    ...
    fn scheduling_task_does_not_change_due_at_and_is_idempotent() {
        ...
        assert_eq!(
            execution::get_task(&connection, &task.id).unwrap().due_at,
            task.due_at
        );
    }
