...
use crate::{
    database::{
        ...
        repositories::{
            ...
            execution_relation::{
                self as repository, CompletionResultRecord, CompletionResultWrite, EntityLinkRecord,
                EntityLinkWrite,
            },
        },
    },
...
fn ensure_execution_entity(
    ...
    if !exists {
        return Err(not_found(format!("关联对象不存在：{entity_type}/{entity_id}")));
    }
    Ok(true)
}
...
pub fn create_link(
    ...
    let source_execution =
        ensure_execution_entity(connection, &user_id, &source_type, &source_id)?;
    let target_execution =
        ensure_execution_entity(connection, &user_id, &target_type, &target_id)?;
...
mod tests {
    use super::*;
    use crate::{
        database::{migration_runner::{run, MigrationContext}, migrations::all},
    ...
    fn database() -> Connection {
        let unique = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let directory = std::env::temp_dir().join(format!("lifetrace-execution-relation-service-{unique}"));
    ...
    fn completion_result_requires_done_task_and_is_editable() {
        ...
        assert!(save_completion_result(&connection, &task.id, CompletionResultInput::default()).is_err());
        execution::change_task_status(&connection, &task.id, TaskStatusInput { status: "done".to_owned() }).unwrap();
        let result = save_completion_result(&connection, &task.id, CompletionResultInput {
    ...
    fn relation_requires_at_least_one_existing_execution_entity() {
        ...
        let link = create_link(&connection, EntityLinkInput {
        ...
        assert_eq!(list_links(&connection, EntityLinksQuery { entity_type: "task".to_owned(), entity_id: task.id }).unwrap().len(), 1);
        assert!(create_link(&connection, EntityLinkInput {
            source_type: "note".to_owned(), source_id: "a".to_owned(), relation_type: "related_to".to_owned(), target_type: "habit".to_owned(), target_id: "b".to_owned()
        }).is_err());
    }
    ...
    fn internal_execution_targets_must_exist() {
        let connection = database();
        let project = execution::create_project(&connection, ProjectInput {
            name: "关联项目".to_owned(), description: None, status: None, color: None, icon: None, sort_order: None,
        }).unwrap();
        assert!(create_link(&connection, EntityLinkInput {
            source_type: "project".to_owned(), source_id: project.id, relation_type: "reference".to_owned(), target_type: "task".to_owned(), target_id: "missing".to_owned()
        }).is_err());
    }
}
