...
pub fn add_subtask(
    ...
    if !structure_repository::set_task_parent(&transaction, &user_id, &child.id, Some(parent_id))
        .map_err(storage)?
    {
...
pub fn add_dependency(
    ...
    let edges =
        structure_repository::list_dependency_edges(connection, &user_id).map_err(storage)?;
...
pub fn list_blockers(connection: &Connection, task_id: &str) -> ExecutionResult<Vec<TaskBlocker>> {
    ...
    for dependency in dependencies {
        let prerequisite =
            task_repository::get_task(connection, &user_id, &dependency.depends_on_task_id)
                .map_err(storage)?;
...
pub fn set_task_recurrence(
    ...
    execution::get_task(connection, task_id)?;
    let current_rule_id =
        structure_repository::task_recurrence_rule_id(connection, &user_id, task_id)
            .map_err(storage)?;
...
pub fn materialize_occurrence(
    ...
    if let Some(existing) =
        structure_repository::get_occurrence_by_key(connection, &user_id, task_id, &occurrence_key)
            .map_err(storage)?
    {
        return Ok(existing);
    }
...
pub fn update_occurrence(
    ...
    let current =
        structure_repository::get_occurrence(connection, &user_id, task_id, occurrence_id)
            .map_err(storage)?
