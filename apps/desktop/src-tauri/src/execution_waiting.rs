use chrono::{Datelike, DateTime, Duration, FixedOffset, Utc};
...
use crate::{
    database::{
        ...
        repositories::{
            ...
            execution_waiting::{self as repository, WaitingItemRecord, WaitingItemWrite, WaitingListFilter},
        },
    },
    execution::{self, ExecutionError, ExecutionErrorKind, ExecutionResult, TaskInput, TaskStatusInput},
};
...
fn parse_now(value: Option<String>) -> ExecutionResult<DateTime<FixedOffset>> {
    match value {
        Some(value) => DateTime::parse_from_rfc3339(&value)
            .map_err(|_| validation("now 必须是 RFC3339 时间")),
...
fn filter_view(
    ...
    let filtered = match view {
        ...
        "cancelled" => items
            ...
            .collect(),
        _ => return Err(validation("view 必须是 all/open/today/overdue/this_week/resolved/cancelled")),
    };
    Ok(filtered)
}
...
pub fn cancel_waiting_item(connection: &Connection, id: &str) -> ExecutionResult<WaitingItemRecord> {
    let user_id = active_user(connection)?;
    let current = repository::get_waiting_item(connection, &user_id, id)
        .map_err(storage)?
        .ok_or_else(|| not_found("等待事项不存在"))?;
...
pub fn create_waiting_from_task(
    ...
    let existing = repository::find_open_by_source_task(&transaction, &user_id, task_id)
        .map_err(storage)?;
...
mod tests {
    ...
    fn waiting_lifecycle_is_explicit_and_idempotent() {
        ...
        let second = resolve_waiting_item(&connection, &item.id, ResolveWaitingInput::default()).unwrap();
    ...
    fn task_to_waiting_is_atomic_and_does_not_duplicate_open_item() {
        ...
        assert_eq!(execution::get_task(&connection, &task.id).unwrap().status, "waiting");
    }
    ...
    fn waiting_to_task_conversion_is_idempotent_and_preserves_links() {
        ...
        assert_eq!(get_waiting_item(&connection, &waiting.id).unwrap().status, "resolved");
