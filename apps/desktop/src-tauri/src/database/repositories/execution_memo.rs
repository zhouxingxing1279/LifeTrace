...
pub fn get(connection: &Connection, user_id: &str, id: &str) -> Result<Option<MemoRecord>, String> {
    let sql = format!(
        "SELECT {COLUMNS} FROM execution_memos WHERE id=?1 AND user_id=?2 AND deleted_at IS NULL"
    );
    let memo = connection
...
pub fn list(
    ...
    let rows = statement
        .query_map(
            params![user_id, filter.status.as_deref(), pinned, search, tag],
            base_from_row,
        )
        .map_err(|error| error.to_string())?;
    let base = rows
        ...
        .map_err(|error| error.to_string())?;
    base.into_iter()
        .map(|memo| hydrate(connection, memo))
        .collect()
}
...
fn replace_tags(
    ...
    connection
        .execute(
            "DELETE FROM execution_memo_tag_relations WHERE memo_id=?1",
            [memo_id],
        )
        .map_err(|error| error.to_string())?;
    for tag in tags {
