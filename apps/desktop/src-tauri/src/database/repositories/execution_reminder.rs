...
pub fn get(connection: &Connection, user_id: &str, id: &str) -> Result<Option<ReminderRecord>, String> {
...
mod tests {
    ...
    fn due_query_uses_snooze_and_fire_key_is_idempotent() {
        ...
        assert!(list_due(&connection, &user_id, "2026-08-08T10:00:00Z", 10).unwrap().is_empty());
        assert_eq!(list_due(&connection, &user_id, "2026-08-08T12:00:00Z", 10).unwrap().len(), 1);
        assert_eq!(find_by_fire_key(&connection, &user_id, &saved.fire_key).unwrap().unwrap().id, saved.id);
    }
}
