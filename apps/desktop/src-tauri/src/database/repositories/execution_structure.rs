...
pub fn count_occurrences(connection: &Connection, user_id: &str, task_id: &str) -> Result<i64, String> {
    connection
        .query_row(
...
pub fn create_occurrence(
    ...
    if let Some(existing) = get_occurrence_by_key(
        connection,
        user_id,
        &input.task_id,
...
mod tests {
    ...
    fn dependency_edges_and_parent_links_round_trip() {
        ...
        assert_eq!(list_dependencies(&connection, &user_id, "child").unwrap().len(), 1);
        assert_eq!(list_dependency_edges(&connection, &user_id).unwrap().len(), 1);
    }
    ...
    fn recurrence_and_occurrence_are_idempotent() {
        ...
        assert_eq!(count_occurrences(&connection, &user_id, "series").unwrap(), 1);
    }
}
