...
mod tests {
    ...
    fn completion_result_upserts_and_soft_deletes() {
        ...
        let first = save_completion_result(
            &connection,
        ...
        .unwrap();
        let second = save_completion_result(
            &connection,
            &CompletionResultWrite {
                user_id: user_id.clone(),
                task_id: "task-1".to_owned(),
        ...
        assert!(get_completion_result(&connection, &user_id, "task-1")
            .unwrap()
            .is_none());
    }
    ...
    fn links_are_bidirectionally_queryable_and_idempotent() {
        ...
        assert_eq!(first.id, second.id);
        assert_eq!(
            list_links_for_entity(&connection, &user_id, "task", "task-a")
                .unwrap()
                .len(),
            1
        );
        assert_eq!(
            list_links_for_entity(&connection, &user_id, "note", "note-b")
                .unwrap()
                .len(),
            1
        );
