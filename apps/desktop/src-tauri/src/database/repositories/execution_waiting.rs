...
mod tests {
    ...
    fn waiting_items_are_profile_scoped_and_soft_deleted() {
        ...
        assert_eq!(
            list_waiting_items(&connection, &user_id, &WaitingListFilter::default())
                .unwrap()
                .len(),
            1
        );
        assert!(get_waiting_item(&connection, "other", &item.id)
            .unwrap()
            .is_none());
        assert!(soft_delete_waiting_item(&connection, &user_id, &item.id).unwrap());
        assert!(get_waiting_item(&connection, &user_id, &item.id)
            .unwrap()
            .is_none());
    }
