...
impl Migration for M0010ExecutionSync {
    ...
    fn up(
        ...
        let specs = [
            ...
            TriggerSpec {
                table: "execution_memo_tag_relations",
                entity_type: "execution.memo_tag_relation",
                id_new: "NEW.memo_id || ':' || NEW.tag_id",
                id_old: "OLD.memo_id || ':' || OLD.tag_id",
                profile_new: "(SELECT user_id FROM execution_memos WHERE id=NEW.memo_id)",
                profile_old: "(SELECT user_id FROM execution_memos WHERE id=OLD.memo_id)",
                delete_new: "0",
            },
            TriggerSpec {
