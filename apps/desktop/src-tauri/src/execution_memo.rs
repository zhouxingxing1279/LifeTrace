...
fn normalize_tags(tags: Vec<String>) -> ExecutionResult<Vec<String>> {
    ...
    if output.len() > 20 {
        return Err(validation("单个备忘录最多 20 个标签"));
    }
    Ok(output)
}
...
fn write_from_input(
    ...
    id: Option<String>,
    input: MemoInput,
    current: Option<&MemoRecord>,
) -> ExecutionResult<MemoWrite> {
...
fn state_write(memo: &MemoRecord, user_id: String, status: &str, pinned: bool) -> MemoWrite {
    MemoWrite {
        id: Some(memo.id.clone()),
        user_id,
        content: memo.content.clone(),
        plain_text: memo.plain_text.clone(),
        is_pinned: pinned,
        status: status.to_owned(),
        archived_at: if status == "archived" {
            memo.archived_at
                .clone()
...
pub fn list_memos(connection: &Connection, query: MemoQuery) -> ExecutionResult<Vec<MemoRecord>> {
    if let Some(status) = query.status.as_deref() {
        if !matches!(status, "active" | "archived") {
            return Err(validation("status 必须是 active 或 archived"));
        }
    }
    ...
    repository::list(
        ...
        &user_id,
        &MemoFilter {
            query: clean_optional(query.q, "搜索词", 500)?,
            status: query.status.or_else(|| Some("active".to_owned())),
            pinned: query.pinned,
            tag: clean_optional(query.tag, "标签", 64)?,
        },
    )
    .map_err(storage)
}
...
pub fn update_memo(
    ...
    id: &str,
    input: MemoInput,
) -> ExecutionResult<MemoRecord> {
    let user_id = active_user(connection)?;
    let current = repository::get(connection, &user_id, id)
        .map_err(storage)?
        .ok_or_else(|| not_found("备忘录不存在"))?;
    if current.status != "active" {
        return Err(conflict("归档备忘录需先恢复后再编辑"));
    }
...
pub fn set_pinned(
    ...
    repository::save(
        connection,
        &state_write(&current, user_id, &current.status, input.pinned),
    )
    .map_err(storage)
}
...
pub fn archive_memo(connection: &Connection, id: &str) -> ExecutionResult<MemoRecord> {
    ...
    repository::save(
        connection,
        &state_write(&current, user_id, "archived", current.is_pinned),
    )
    .map_err(storage)
}
...
pub fn restore_memo(connection: &Connection, id: &str) -> ExecutionResult<MemoRecord> {
    ...
    repository::save(
        connection,
        &state_write(&current, user_id, "active", current.is_pinned),
    )
    .map_err(storage)
}
...
fn archive_in_transaction(
    ...
    repository::save(
        connection,
        &state_write(memo, user_id, "archived", memo.is_pinned),
    )
    .map_err(storage)?;
    Ok(())
}
...
pub fn convert_to_task(
    connection: &Connection,
    memo_id: &str,
    input: MemoToTaskInput,
) -> ExecutionResult<crate::database::repositories::execution::TaskRecord> {
    let user_id = active_user(connection)?;
    let memo = repository::get(connection, &user_id, memo_id)
        .map_err(storage)?
        .ok_or_else(|| not_found("备忘录不存在"))?;
    if let Some(target) = repository::find_conversion_target(connection, &user_id, memo_id, "task")
        .map_err(storage)?
    {
        return execution::get_task(connection, &target)
            .map_err(|_| conflict("备忘录已转换过，但目标任务不存在"));
    }
    let title =
        clean_optional(input.title, "任务标题", 240)?.unwrap_or_else(|| default_title(&memo));
    let task_input = TaskInput {
        project_id: input.project_id,
        title,
        description: clean_optional(input.description, "任务描述", 20_000)?
            .or_else(|| Some(memo.content.clone())),
        ...
        context: input.context.or_else(|| memo.context.clone()),
    };
    let transaction = connection
        .unchecked_transaction()
        .map_err(|error| storage(error.to_string()))?;
...
pub fn convert_to_calendar(
    connection: &Connection,
    memo_id: &str,
    input: MemoToCalendarInput,
    ...
    let memo = repository::get(connection, &user_id, memo_id)
        ...
        .ok_or_else(|| not_found("备忘录不存在"))?;
    if let Some(target) =
        repository::find_conversion_target(connection, &user_id, memo_id, "calendar_event")
            .map_err(storage)?
    {
        return execution_calendar::get_event(connection, &target)
            .map_err(|_| conflict("备忘录已转换过，但目标日历事件不存在"));
    }
    let target_input = CalendarEventInput {
        title: clean_optional(input.title, "事件标题", 240)?
            .unwrap_or_else(|| default_title(&memo)),
        description: clean_optional(input.description, "事件描述", 20_000)?
            .or_else(|| Some(memo.content.clone())),
        is_all_day: input.timing.is_all_day,
        start_at: input.timing.start_at,
        end_at: input.timing.end_at,
        start_local_date: input.timing.start_local_date,
        end_local_date: input.timing.end_local_date,
...
pub fn convert_to_waiting(
    connection: &Connection,
    memo_id: &str,
    input: MemoToWaitingInput,
) -> ExecutionResult<crate::database::repositories::execution_waiting::WaitingItemRecord> {
    let user_id = active_user(connection)?;
    let memo = repository::get(connection, &user_id, memo_id)
        .map_err(storage)?
        .ok_or_else(|| not_found("备忘录不存在"))?;
    if let Some(target) =
        repository::find_conversion_target(connection, &user_id, memo_id, "waiting_item")
            .map_err(storage)?
    {
        return execution_waiting::get_waiting_item(connection, &target)
            .map_err(|_| conflict("备忘录已转换过，但目标等待事项不存在"));
    }
    let target_input = WaitingItemInput {
        title: clean_optional(input.title, "等待事项标题", 240)?
            .unwrap_or_else(|| default_title(&memo)),
...
mod tests {
    ...
    fn db() -> Connection {
        let unique = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("lifetrace-memo-service-{unique}"));
    ...
    fn memo_input() -> MemoInput {
        MemoInput {
            content: "Call Alice about contract".to_owned(),
            context: Some("work".to_owned()),
            tags: vec!["Follow-up".to_owned()],
        }
    }
    #[test]
    fn memo_lifecycle_and_search_work() {
        let connection = db();
        let memo = create_memo(&connection, memo_input()).unwrap();
        assert_eq!(
            list_memos(
                &connection,
                MemoQuery {
                    q: Some("follow-up".to_owned()),
                    ..Default::default()
                }
            )
            .unwrap()
        ...
        let archived = archive_memo(&connection, &memo.id).unwrap();
        assert_eq!(archived.status, "archived");
        assert!(list_memos(&connection, MemoQuery::default())
            .unwrap()
            .is_empty());
        assert_eq!(
            restore_memo(&connection, &memo.id).unwrap().status,
            "active"
        );
    }
    #[test]
    fn memo_to_task_is_atomic_idempotent_and_archives_source() {
        let connection = db();
        let memo = create_memo(&connection, memo_input()).unwrap();
        let input = MemoToTaskInput {
            project_id: None,
            title: None,
            description: None,
        ...
        let first = convert_to_task(&connection, &memo.id, input.clone()).unwrap();
        let second = convert_to_task(&connection, &memo.id, input).unwrap();
        assert_eq!(first.id, second.id);
        assert_eq!(get_memo(&connection, &memo.id).unwrap().status, "archived");
        let links:i64=connection.query_row("SELECT COUNT(*) FROM execution_entity_links WHERE (source_id=?1 OR target_id=?1) AND deleted_at IS NULL",[memo.id],|row|row.get(0)).unwrap();
        assert_eq!(links, 2);
    }
    #[test]
    fn memo_can_receive_shared_reminder() {
        let connection = db();
        let memo = create_memo(&connection, memo_input()).unwrap();
        let reminder = crate::execution_reminder::create_reminder(
            &connection,
            crate::execution_reminder::ReminderInput {
                subject_type: "memo".to_owned(),
                subject_id: memo.id,
                trigger_at: "2026-08-10T09:00:00+08:00".to_owned(),
                timezone: Some("Asia/Shanghai".to_owned()),
            },
        )
        .unwrap();
        assert_eq!(reminder.subject_type, "memo");
    }
}
