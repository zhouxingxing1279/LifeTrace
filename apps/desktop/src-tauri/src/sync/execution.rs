...
use rusqlite::{params, params_from_iter, types::{Value as SqlValue, ValueRef}, Connection, OptionalExtension};
...
const TABLE_SPECS: &[TableSpec] = &[
    TableSpec { entity_type: "execution.project", table: "execution_projects", soft_delete: true },
    TableSpec { entity_type: "execution.recurrence_rule", table: "execution_recurrence_rules", soft_delete: true },
    TableSpec { entity_type: "execution.task", table: "execution_tasks", soft_delete: true },
    TableSpec { entity_type: "execution.task_dependency", table: "execution_task_dependencies", soft_delete: false },
    TableSpec { entity_type: "execution.task_occurrence", table: "execution_task_occurrences", soft_delete: true },
    TableSpec { entity_type: "execution.waiting_item", table: "execution_waiting_items", soft_delete: true },
    TableSpec { entity_type: "execution.calendar_event", table: "execution_calendar_events", soft_delete: true },
    TableSpec { entity_type: "execution.calendar_occurrence", table: "execution_calendar_occurrences", soft_delete: true },
    TableSpec { entity_type: "execution.memo", table: "execution_memos", soft_delete: true },
    TableSpec { entity_type: "execution.memo_tag", table: "execution_memo_tags", soft_delete: true },
    TableSpec { entity_type: "execution.reminder", table: "execution_reminders", soft_delete: true },
    TableSpec { entity_type: "execution.completion_result", table: "execution_completion_results", soft_delete: true },
    TableSpec { entity_type: "execution.entity_link", table: "execution_entity_links", soft_delete: true },
];
...
fn apply_normal_upsert(
    ...
    let columns = table_columns
        ...
        .collect::<Vec<_>>();
    if columns.is_empty() {
        return Err(format!("{} payload has no writable columns", spec.entity_type));
    }
...
mod tests {
    ...
    fn memo_tag_relation_and_soft_delete_round_trip() {
        ...
        assert!(load_local_entity(&connection, &profile, MEMO_TAG_RELATION, "m1:tag1")
            .unwrap()
            .is_some());
