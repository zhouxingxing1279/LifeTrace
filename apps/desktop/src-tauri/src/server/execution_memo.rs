...
pub async fn list(State(state): State<AppState>, Query(query): Query<MemoQuery>) -> Response {
    let items = with_db!(state, |db| execution_memo::list_memos(db, query));
    Json(items).into_response()
}
pub async fn get(State(state): State<AppState>, Path(id): Path<String>) -> Response {
    let item = with_db!(state, |db| execution_memo::get_memo(db, &id));
    Json(item).into_response()
}
pub async fn create(State(state): State<AppState>, Json(input): Json<MemoInput>) -> Response {
    let item = with_db!(state, |db| execution_memo::create_memo(db, input));
...
pub async fn update(
    ...
    let item = with_db!(state, |db| execution_memo::update_memo(db, &id, input));
    Json(item).into_response()
}
pub async fn pin(
    State(state): State<AppState>,
    Path(id): Path<String>,
    Json(input): Json<PinMemoInput>,
) -> Response {
    let item = with_db!(state, |db| execution_memo::set_pinned(db, &id, input));
    Json(item).into_response()
}
pub async fn archive(State(state): State<AppState>, Path(id): Path<String>) -> Response {
    let item = with_db!(state, |db| execution_memo::archive_memo(db, &id));
    Json(item).into_response()
}
pub async fn restore(State(state): State<AppState>, Path(id): Path<String>) -> Response {
    let item = with_db!(state, |db| execution_memo::restore_memo(db, &id));
    Json(item).into_response()
}
pub async fn delete(State(state): State<AppState>, Path(id): Path<String>) -> Response {
    with_db!(state, |db| execution_memo::delete_memo(db, &id));
    Json(OkResponse { ok: true }).into_response()
}
pub async fn convert_to_task(
    State(state): State<AppState>,
    Path(id): Path<String>,
    Json(input): Json<MemoToTaskInput>,
) -> Response {
    let item = with_db!(state, |db| execution_memo::convert_to_task(db, &id, input));
    Json(item).into_response()
}
pub async fn convert_to_calendar(
    State(state): State<AppState>,
    Path(id): Path<String>,
    Json(input): Json<MemoToCalendarInput>,
) -> Response {
    let item = with_db!(state, |db| execution_memo::convert_to_calendar(
        db, &id, input
    ));
    Json(item).into_response()
}
pub async fn convert_to_waiting(
    State(state): State<AppState>,
    Path(id): Path<String>,
    Json(input): Json<MemoToWaitingInput>,
) -> Response {
    let item = with_db!(state, |db| execution_memo::convert_to_waiting(
        db, &id, input
    ));
    Json(item).into_response()
}
