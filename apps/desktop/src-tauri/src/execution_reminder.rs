...
fn valid_subject_type(subject_type: &str) -> bool {
    matches!(subject_type, "task" | "calendar_event" | "waiting_item" | "memo")
}
...
fn subject_exists(
    ...
    let table = match subject_type {
        ...
        _ => return Err(validation("subjectType 必须是 task/calendar_event/waiting_item/memo")),
    };
    let sql = format!("SELECT EXISTS(SELECT 1 FROM {table} WHERE id=?1 AND user_id=?2 AND deleted_at IS NULL)");
    connection
...
pub fn list_subject_reminders(
    ...
    if !valid_subject_type(&subject_type) {
        return Err(validation("subjectType 必须是 task/calendar_event/waiting_item/memo"));
    }
...
pub fn create_reminder(connection: &Connection, input: ReminderInput) -> ExecutionResult<ReminderRecord> {
    let user_id = active_user(connection)?;
    ...
    if let Some(existing) = repository::find_by_fire_key(connection, &user_id, &fire_key).map_err(storage)? {
        return Ok(existing);
    }
    repository::save(
        connection,
...
pub fn update_reminder(
    ...
    if let Some(existing) = repository::find_by_fire_key(connection, &user_id, &key).map_err(storage)? {
...
pub fn snooze_reminder(
    ...
    if DateTime::parse_from_rfc3339(&until_at).unwrap().with_timezone(&Utc) <= Utc::now() {
        return Err(validation("untilAt 必须晚于当前时间"));
    }
    transition(connection, id, "scheduled", Some(until_at), current.last_fired_at)
}
...
pub fn delete_reminder(connection: &Connection, id: &str) -> ExecutionResult<()> {
    let user_id = active_user(connection)?;
    if repository::get(connection, &user_id, id).map_err(storage)?.is_none() {
...
mod tests {
    ...
    fn due_query_and_lifecycle_do_not_duplicate_fire() {
        ...
        assert!(list_due_reminders(
            &connection,
            DueReminderQuery { now: Some("2026-08-09T00:59:59Z".to_owned()), limit: None }
        ).unwrap().is_empty());
        assert_eq!(list_due_reminders(
            &connection,
            DueReminderQuery { now: Some("2026-08-09T01:00:00Z".to_owned()), limit: None }
        ).unwrap().len(), 1);
        ...
        assert!(list_due_reminders(
            &connection,
            DueReminderQuery { now: Some("2026-08-10T00:00:00Z".to_owned()), limit: None }
        ).unwrap().is_empty());
    }
